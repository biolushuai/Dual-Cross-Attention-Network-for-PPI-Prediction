...
dataset_path = '/mnt/d/ProjectsData/CollaPPI/'

def train_nn(model, train_loader, test_loader, device, args, fold_num):
    ...
    for e in range(Epoch):
        ...
        for batch_idx_train,batch in enumerate(tqdm(train_loader,mininterval=0.5,desc='Training',leave=False,ncols=50)):
            ...
            # interaction_loss = celoss(predicted_label, true_label)
        ...
        test_acc, test_prec, test_recall, test_f1, test_auc, con_matrix,test_spec,test_mcc, test_pr = cal_metric(total_preds,total_labels,total_scores,args)
        ...
        if test_f1>= best_test_f1:
            ...
            model_name = '-'.join([str(i)[:7] for i in best_record])
            torch.save(model.state_dict(), dataset_path + args.dataset + '/dictionary/models/5-Folds/fold' + fold_num + '-' + model_name + '.pt')
...
def load_files(dataset='yeast'):
    ...
    contact_path = dataset_path+dataset+'/dictionary/contact_map_'+args.distance+'/'
    ...
    lm_path = dataset_path+dataset+'/dictionary/pretrained-emb-'+args.lm+'/'
    ...
    ###########load GO-terms label
    GO_terms_path = dataset_path+dataset+'/dictionary/GO_terms_label/'
    ...
    ###########
    ############load location label
    location_path = dataset_path+dataset+'/dictionary/location_label/'
...
def train(args):
    ...
    for i in range(5):
        ...
        valid_data = PPIDataset(data=valid_items, contact_dic=contact_dic, lm_dic=lm_dic, GO_dic=GO_dic, location_dic=location_dic)
        ...
        best_record = train_nn(model=model, train_loader=dataset_train, test_loader=dataset_valid, device=device, args=args, fold_num=str(i+1))
    ...
    title = 'ACC Precision Recall Specificity F1 MCC AUC'
    print(mean_values)
    print(std_values)
    with open(dataset_path+'/yeast/dictionary/models/5-Folds/5_fold_'+args.lm+'_'+args.distance+'.txt','a') as f:
        f.write(title+'\n'+str(mean_values)+'\n'+str(std_values))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--distance', type=str, default='8.0', help="distance threshold (4.0/6.0/8.0/10.0) for contact map")
