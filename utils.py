...
def load_dataset(dataset='yeast',mark='train',args=None):
    ...
    if dataset != 'multi_species':
        with open(dataset_path+dataset+'/actions/'+mark+'_cmap.actions.tsv') as f:
        # with open(dataset_path + dataset + '/actions/' + mark + '.tsv') as f:
            for line in f:
        ...
        with open(dataset_path+dataset+'/actions/'+mark+'-multi-'+args.identity+'.tsv') as f:
            for line in f:
                prot1 = line.strip().split(split_mark)[0]
                prot2 = line.strip().split(split_mark)[1]
                interaction = line.strip().split(split_mark)[2]
...
def load_files(dataset='yeast'):
    ...
    # elif dataset == 'multi_class':
    split_mark = '.npy'
    contact_path = dataset_path+dataset+'/dictionary/contact_map_8.0/'
    ...
    lm_path = dataset_path+dataset+'/dictionary/pretrained-emb-esm-2/'
